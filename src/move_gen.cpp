...
unsigned int MoveGen::get_move(){
    ...
    //TODO: Add generation for knight, bishop, and rook promotion
    if(piece == pPAWN && is_final_rank(side, to)) {
        if(additional_info == CAPTURE)
...
bool MoveGen::is_final_rank(int side, int sq){
    return side == WHITE ? sq >= 56 : sq <= 7;
}
